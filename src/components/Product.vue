  ...
  <div>
    <div class="relative">
      <img class="h-[20vh] w-full rounded-xl border-2 object-cover lg:h-[25vh]"
        :class="index === 1 ? 'border-red' : 'border-transparent'" :alt=product.name :src=product.image.desktop />
    ...
    <div class="mt-12 flex flex-col gap-1">
      ...
      <p class="price font-semibold text-red">{{ formatter.format(product.price) }}</p>
    </div>
  </div>
</template>